...
public class OpCommand extends AbstractCommand implements ICommandPlugin {
	private boolean isActive = false;
	private String name = "OP";
	private String desc = "Sets a person to operator.";
	private boolean isOpCommand = true;

	private List<String> aliases = Arrays.asList("op", "OP");
	...
	public void onCommand(MessageReceivedEvent e, String[] args) {
		...
		if (isActive) {
			...
			if (!isOpCommand || PermissionHandler.getInstance().isOP(e.getAuthor())) {
				Log.log(BotDefs.TYPE_COMMAND, true, e.getAuthor().getName() + " issued an OP command!");
				PermissionHandler.getInstance().addOP(args[1]);
				Log.log(BotDefs.TYPE_COMMAND, true, args[1] + " was set to OP!");
			}
		}
	}
