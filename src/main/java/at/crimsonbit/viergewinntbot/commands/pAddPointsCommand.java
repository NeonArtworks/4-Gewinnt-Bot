...
public class pAddPointsCommand extends AbstractCommand implements ICommandPlugin {
	private boolean isActive = false;
	private String name = "pAddPoints";
	private String desc = "Adds points to a user int the PointList";
	private boolean isOpCommand = true;

	private List<String> aliases = Arrays.asList("pAddPoints", "paddpoints");
	...
	public void onCommand(MessageReceivedEvent e, String[] args) {
		if (isActive) {
			...
			if (!isOpCommand || PermissionHandler.getInstance().isOP(e.getAuthor())) {
				...
				pList.addPointsToUser(e.getAuthor(), 10);
